...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Pas 5: producció de dinàmica molecular.")
    parser.add_argument("--input-pdb", type=Path, default=Path("runs/04_npt/npt_final.pdb"))
    parser.add_argument("--output-dir", type=Path, default=Path("runs/05_production"))
...
def main() -> None:
    ...
    attach_reporters(simulation, args.output_dir, args.steps, args.report_interval, "production")
    simulation.step(args.steps)

    positions = simulation.context.getState(getPositions=True).getPositions()
    out_pdb = args.output_dir / "production_final.pdb"
    write_pdb(simulation.topology, positions, out_pdb)
    (args.output_dir / "summary.txt").write_text(summarize_energy(simulation) + "\n", encoding="utf-8")
    print(f"Producció completada a {out_pdb}")
